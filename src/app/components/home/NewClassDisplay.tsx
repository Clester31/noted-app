        ...
        <div className="absolute inset-0 flex justify-center items-center backdrop-blur-md bg-black/30">
            <div className="w-1/4 bg-zinc-800 p-4 rounded-xl" style={{borderBottom: `10px solid #${classColor}`}}>
                ...
                <div className="text-center">
                    ...
                    <input type="text" name="newClassName" placeholder="Class Name" className="bg-zinc-500 rounded mb-4 h-8 p-2" onChange={(e) => setClassName(e.target.value)} />
                    <p className="mb-4">Select a color</p>
                    <div className="grid grid-cols-5 gap-4">
                        {colors.map((color: string, idx: number) => {
                            return (
                                <div key={idx} style={{ backgroundColor: `#${color}` }} className={`w-8 h-8 rounded-full cursor-pointer ${classColor === color && `border-2 border-white`}`} onClick={() => setClassColor(color)}>
                                </div>
                            )
                        })}
                    </div>
                    <button className="flex m-auto p-2 rounded mt-4 bg-zinc-500" onClick={handleSubmit}>Add Class</button>
                </div>
            </div>
        </div>
    )
